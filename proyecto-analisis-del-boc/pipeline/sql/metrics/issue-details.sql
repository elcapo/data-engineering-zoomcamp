...
LEFT JOIN boc_dataset.download_log AS dl ON dl.entity_type = 'document' AND dl.year = i.year AND dl.issue = i.issue AND dl.disposition = (d._dlt_list_idx + 1)
LEFT JOIN boc_dataset.extraction_log AS el ON el.entity_type = 'document'AND el.year = i.year AND el.issue = i.issue AND el.disposition = (d._dlt_list_idx + 1)
