...
def load_creds(root: Path) -> tuple[str, str, str]:
    env = load_env(root)
    host = f"http://localhost:{env.get('METABASE_PORT', '3000')}"
    user = env.get("METABASE_ADMIN_EMAIL", "admin@climate.local")
    password = env.get("METABASE_ADMIN_PASSWORD")
    if not password:
        sys.exit("METABASE_ADMIN_PASSWORD not set (in .env or environment)")
